...
<head>
  ...
  <title>Night Sky Futuristic Payroll Calculator — Fixed</title>
  <style>
    :root{
...
<div class="wrap">
  <div class="card">
    ...
    <div class="row">
      ...
      <button class="small-btn" onclick="exportCSV()">📤 Ekspor CSV</button>
      <label class="small-btn" style="display:flex;align-items:center;gap:6px">
        📂 Impor CSV <input id="importFile" type="file" accept=".csv" style="display:none" onchange="importCSV(event)">
      </label>
      <button class="small-btn" onclick="clearAll()">🗑️ Hapus Semua</button>
      
      <button class="small-btn" onclick="copyResults()">📋 Salin Hasil</button>
      <button class="small-btn" onclick="printSlip()">🖨️ Cetak Slip</button>
    </div>
